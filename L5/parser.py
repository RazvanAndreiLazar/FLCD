...
if __name__ == '__main__':
    grammar_in = Grammar()
    grammar_no = 2
    program_particularity = ''
    filename = f'L5/g{str(grammar_no) + program_particularity}.txt'
    ...
    pr = PifReader()
    pr.readPIF(f'L3/PIF{program_particularity}.out')

    po = parser.parse(pr.get_keys())
    print('Parsing output:', po, sep='\n')
    po.print_to_file(f'L5/out/g{str(grammar_no) + program_particularity}.out')
